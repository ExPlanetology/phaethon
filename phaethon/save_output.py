"""
Part of the FastChem 3.1 package.

Copyright Daniel Kitzmann, Joachim Stock

https://github.com/NewStrangeWorlds/FastChem/blob/master/python/save_output.py
"""
...
def saveChemistryOutput(
    ...
    temperature,
    pressure,  # arrays of temperature and pressure
...
):  # the header descriptions of the additional columns
    ...
    if (nb_add_columns > 0 and add_columns_desc.size != nb_add_columns) or (
        additional_columns_desc is None and nb_add_columns > 0
    ):
        print(
            "Warning from saveChemistryOutput: The number of additional column descriptions does not match the number of data columns."
        )
    ...
    with open(file_path, "w") as file:
        # file header
        file.write(
            "{0:<16}\t{1:<16}\t{2:<16}\t{3:<16}\t{4:<16}".format(
                "#p(bar)", "T(K)", "n_<tot>(cm-3)", "n_g(cm-3)", "m(u)"
            )
        )
        ...
        if output_species is None:  # save all species
            ...
            for i in range(pressure.size):
                file.write(
                    "{0:1.10e}\t{1:1.10e}\t{2:1.10e}\t{3.value:1.10e}\t{4:1.10e}".format(
                        pressure[i],
                        temperature[i],
            ...
            for species in output_species:
                if (
                    fastchem.getGasSpeciesIndex(species)
                    != pyfastchem.FASTCHEM_UNKNOWN_SPECIES
                ):
                    ...
                    print(
                        "Species ",
                        species,
                        " not found during saving of the chemistry output!",
                    )
            ...
            for i in range(pressure.size):
                ...
                for species in output_species:
                    ...
                    if species_index != pyfastchem.FASTCHEM_UNKNOWN_SPECIES:
                        file.write(
                            "\t{0:1.10e}".format(mixing_ratios[i, species_index])
                        )
...
def saveCondOutput(
    ...
    if (nb_add_columns > 0 and add_columns_desc.size != nb_add_columns) or (
        ...
        print(
            "Warning from saveCondOutput: The number of additional column descriptions does not match the number of data columns."
        )
    ...
    with open(file_path, "w") as file:
        ...
        if output_species is None:  # save all species
            ...
            for species in output_species:
                if (
                    fastchem.getCondSpeciesIndex(species)
                    != pyfastchem.FASTCHEM_UNKNOWN_SPECIES
                ):
                    file.write("\t{0:<16}".format(species))
                else:
            ...
            for i in range(pressure.size):
                ...
                for species in output_species:
                    ...
                    if species_index != pyfastchem.FASTCHEM_UNKNOWN_SPECIES:
                        file.write(
                            "\t{0:1.10e}".format(number_densities[i, species_index])
                        )
...
def saveMonitorOutput(
    ...
    if (nb_add_columns > 0 and add_columns_desc.size != nb_add_columns) or (
        ...
        print(
            "Warning from saveMonitorOutput: The number of additional column descriptions does not match the number of data columns."
        )
    ...
    with open(file_path, "w") as file:
        # file header
        file.write(
            "{0:<16}{1:<16}{2:<16}{3:<16}{4:<24}{5:<24}{6:<24}{7:<24}{8:<24}{9:<24}{10:<24}".format(
                "#grid_point",
                "#iterations",
                "#chem_iter",
                "#cond_iter",
        ...
        for i in range(len(fastchem_flags)):
            ...
            if np.isin(0, element_conserved[i], assume_unique=True):
                all_elements_conserved = 0

            file.write(
                "{0:<16d}{1:<16d}{2:<16d}{3:<16d}{4:<24s}{5:<24s}{6:<24.10e}{7:<24.10e}{8:<24.10e}{9.value:<24.10e}{10:<24.10e}".format(
                    i,
                    nb_iterations[i],
...
def saveChemistryOutputPandas(
    ...
    if (add_columns_desc.size != nb_add_columns and nb_add_columns > 0) or (
        additional_columns_desc is None and nb_add_columns > 0
    ):
        print(
            "Warning from saveChemistryOutputPandas: The number of additional column descriptions does not match the number of data columns."
        )
    ...
    for i in range(nb_add_columns):
        if (
            add_columns_desc.size == nb_add_columns
            and additional_columns_desc is not None
        ):
...
def saveCondOutputPandas(
    ...
    # combine all general data columns
    if additional_columns is None:
        rows = np.vstack([pressure, temperature]).T
    else:
        rows = np.vstack([pressure, temperature, additional_columns]).T
    ...
    if output_species is None:
        ...
        data = np.hstack(
            [rows, element_cond_degree, number_densities_cond[:, select_species_id]]
        )
...
def saveMonitorOutputPandas(
    ...
    if (add_columns_desc.size != nb_add_columns and nb_add_columns > 0) or (
        ...
        print(
            "Warning from saveMonitorOutputPandas: The number of additional column descriptions does not match the number of data columns."
        )
